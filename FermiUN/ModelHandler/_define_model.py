...
def define_model(self):
    '''
    This method builds a 4-story Unet model. 
    The model structure is taken from absDL.
    '''
    d1 = 64
    d2 = 128
    d3 = 256
    ...
    cropL = int((lastS-self.config.radius * 2) / 2)
    cropL1 = int(crop[0])
    cropL2 = int(crop[1])
    cropL3 = int(crop[2])
    cropL4 = int(crop[3])

    inp = Input(shape=(self.config.image.width,self.config.image.width))
    ...
    output = Reshape((self.config.radius * 2,self.config.radius * 2))(output)
